...
describe('getBestMove', () => {
  ...
  DIFFICULTIES.forEach((lv) => {
    test(`Lv${lv}: 空の盤面 → 合法手を返す`, () => {
      ...
      const move = getBestMove(board, 'blue', SIZE, lv);
    ...
    test(`Lv${lv}: 合法手ゼロ → null を返す`, () => {
      ...
      expect(getBestMove(board, 'blue', SIZE, lv)).toBeNull();
    });
  ...
  describe('Lv1 (かんたん)', () => {
    test('rand=0 で常に同じ手（決定論的）', () => {
      const board = createEmptyBoard(SIZE);
      const rand = () => 0;
      expect(getBestMove(board, 'blue', SIZE, 1, rand))
        .toEqual(getBestMove(board, 'blue', SIZE, 1, rand));
    });
  });
  ...
  describe('Lv2 (ふつう)', () => {
    test('最大支配マス数を持つ手を選ぶ', () => {
      const board = createEmptyBoard(SIZE);
      const move = getBestMove(board, 'blue', SIZE, 2);
      expect(move).not.toBeNull();

      // 得られた手のスコア
      const score = scoreMove(board, move!, 'blue', SIZE, /* includePower */ false);

      // 角(0,0)のweakは支配3マス → Lv2 はこれより高スコアの手を選ぶはず
      const cornerScore = scoreMove(board, { type: 'build', row: 0, col: 0, shape: 'weak' }, 'blue', SIZE, false);
    ...
    test('rand=0 で同じ手（決定論的）', () => {
      const board = createEmptyBoard(SIZE);
      const rand = () => 0;
      expect(getBestMove(board, 'blue', SIZE, 2, rand))
        .toEqual(getBestMove(board, 'blue', SIZE, 2, rand));
    });
  });
  ...
  describe('Lv3 (むずかしい)', () => {
    test('合法手の中で最高スコアを持つ手を返す', () => {
      const board = createEmptyBoard(SIZE);
      const move = getBestMove(board, 'blue', SIZE, 3);
      expect(move).not.toBeNull();

      const score = scoreMove(board, move!, 'blue', SIZE, true);

      // 全合法手のスコアを確認し、返された手が最高（or 同等）であること
      // ここでは角との比較で十分
      const cornerScore = scoreMove(board, { type: 'build', row: 0, col: 0, shape: 'weak' }, 'blue', SIZE, true);
      expect(score).toBeGreaterThanOrEqual(cornerScore);
    });
    ...
    test('Lv3 が Lv2 より強い（ある局面でスコアが同等以上）', () => {
      ...
      const board = createEmptyBoard(SIZE);
      const moveLv2 = getBestMove(board, 'blue', SIZE, 2, () => 0);
      const moveLv3 = getBestMove(board, 'blue', SIZE, 3);
      expect(moveLv3).not.toBeNull();
      expect(moveLv2).not.toBeNull();
      // Lv3 の手のスコアは Lv2 の手のスコア以上
      const scoreLv3 = scoreMove(board, moveLv3!, 'blue', SIZE, true);
      const scoreLv2 = scoreMove(board, moveLv2!, 'blue', SIZE, true);
  ...
  describe('Lv4 (さいきょう)', () => {
    test('空の盤面 → 合法手を返す', () => {
      const board = createEmptyBoard(SIZE);
      const move = getBestMove(board, 'blue', SIZE, 4);
      expect(move).not.toBeNull();
      expect(isLegalMove(board, move!, 'blue', SIZE)).toBe(true);
    });
  ...
  test('盤面が埋まりかけの局面でも全難易度が合法手を返す', () => {
    ...
    board = applyAction(board, { type: 'build', row: 0, col: 5, shape: 'weak' }, 'blue');

    for (const lv of [1, 2, 3, 4] as const) {
      const move = getBestMove(board, 'red', SIZE, lv);
      expect(move).not.toBeNull();
      expect(isLegalMove(board, move!, 'red', SIZE)).toBe(true);
    }
  });
});
