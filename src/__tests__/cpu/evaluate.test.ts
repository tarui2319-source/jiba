...
describe('scoreBoard', () => {
  ...
  test('空の盤面: blue / red ともにスコア 0', () => {
    ...
    expect(scoreBoard(inf, 'first')).toBeCloseTo(0);
    expect(scoreBoard(inf, 'second')).toBeCloseTo(0);
  });
  ...
  test('blue が 1マス配置後: blue スコア > 0, red スコア < 0', () => {
    ...
    const inf = computeInfluence(board, SIZE);
    expect(scoreBoard(inf, 'first')).toBeGreaterThan(0);
    expect(scoreBoard(inf, 'second')).toBeLessThan(0);
  });
  ...
  test('blue と red のスコアは符号反転（ゼロサム）', () => {
    ...
    board = applyAction(board, { type: 'build', row: 3, col: 3, shape: 'weak' }, 'first');
    const inf = computeInfluence(board, SIZE);
    expect(scoreBoard(inf, 'first')).toBeCloseTo(-scoreBoard(inf, 'second'));
  });
  ...
  test('includePower=false: 支配マス差のみ（整数値）', () => {
    ...
    board = applyAction(board, { type: 'build', row: 3, col: 3, shape: 'weak' }, 'first');
    const inf = computeInfluence(board, SIZE);
    const score = scoreBoard(inf, 'first', false);
  ...
  test('includePower=true は includePower=false より精度が高い（float）', () => {
    ...
    board = applyAction(board, { type: 'build', row: 3, col: 3, shape: 'weak' }, 'first');
    const inf = computeInfluence(board, SIZE);
    const withPower    = scoreBoard(inf, 'first', true);
    const withoutPower = scoreBoard(inf, 'first', false);
    // Δ影響力の 0.01 係数分だけ差がある
    expect(withPower).not.toBe(withoutPower);
  });

  test('strong_vert は weak より blue スコアが高い（中央配置）', () => {
    const boardWeak = applyAction(createEmptyBoard(SIZE), { type: 'build', row: 3, col: 3, shape: 'weak' }, 'first');
    const boardStrong = applyAction(createEmptyBoard(SIZE), { type: 'build', row: 3, col: 3, shape: 'strong_vert' }, 'first');
    ...
    expect(scoreBoard(infWeak, 'first', false)).toBeGreaterThan(scoreBoard(infStrong, 'first', false));
    expect(scoreBoard(infWeak, 'first', true)).toBeGreaterThan(0);
    expect(scoreBoard(infStrong, 'first', true)).toBeGreaterThan(0);
  });
});
...
describe('scoreMove', () => {
  ...
  test('中央付近の weak は角の weak より高スコア', () => {
    const board = createEmptyBoard(SIZE);
    const centerScore = scoreMove(board, { type: 'build', row: 2, col: 2, shape: 'weak' }, 'first', SIZE);
    const cornerScore = scoreMove(board, { type: 'build', row: 0, col: 0, shape: 'weak' }, 'first', SIZE);
  ...
  test('blue の scoreMove は red から見ると低スコア', () => {
    ...
    const redScore = scoreMove(board, { type: 'build', row: 3, col: 3, shape: 'weak' }, 'second', SIZE);
    expect(redScore).toBeGreaterThan(0);
    expect(blueScore).toBeCloseTo(-scoreMove(createEmptyBoard(SIZE), { type: 'build', row: 3, col: 3, shape: 'weak' }, 'first', SIZE) * -1);
  });
  ...
  test('角マス(0,0)での weak は 4 マス支配（アンカーマス自身 + 隣接3マス）', () => {
    const board = createEmptyBoard(SIZE);
    const score = scoreMove(board, { type: 'build', row: 0, col: 0, shape: 'weak' }, 'first', SIZE, false);
  ...
  test('中央(2,2)での weak は 9 マス支配（アンカーマス自身 + 周囲8マス）', () => {
    const board = createEmptyBoard(SIZE);
    const score = scoreMove(board, { type: 'build', row: 2, col: 2, shape: 'weak' }, 'first', SIZE, false);
    expect(score).toBe(9);
  });
});
